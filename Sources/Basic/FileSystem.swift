...
public enum FileMode {
    ...
    public var setMode: (Int16) -> Int16 {
        switch self {
        case .userUnWritable:
            // r-x rwx rwx
            return {$0 & 0o577}
...
private class LocalFileSystem: FileSystem {
    ...
    func chmod(_ mode: FileMode, path: AbsolutePath, options: Set<FileMode.Option>) throws {
        func setMode(url: URL) throws {
            // Skip if only files should be changed.
            if options.contains(.onlyFiles) && isDirectory(AbsolutePath(url.path)) {
                return
            }
        ...
        let urlPath = URL(fileURLWithPath: path.pathString)
        try setMode(url: urlPath)
        guard isDirectory(AbsolutePath(urlPath.path)) else { return }
        ...
        while let p = traverse.nextObject() {
            guard let url = p as? URL else {
                throw FileSystemError.noEntry
            }
            try setMode(url: url)
        }
